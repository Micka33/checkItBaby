...
{
    return JSON.stringify({
      message: $('#content .error_page_message').text()
    });
};
var getPaging = function()
{
    return JSON.stringify({
      ...
      done: (($('#charms .nav-pager .pager-next span.off').length > 0) ? ('done') : (null))
    });
};
...
var visit = function()
{
    ...
    this.waitForSelector(selector, function()
    {
      this.captureSelector(current.toString()+'_'+guy.name.replace(/\/|\\/, '')+'.jpeg', selector, {format: 'jpeg', quality: 100});
    }, null, 12000);
}
var visitProfiles = function()
{
  ...
  if (JSON.parse(this.evaluate(getPaging)).done === 'done')
  {
    log('Tous les profiles ont été visités.');
...
var getSearchResults = function()
{
  ...
  var sel = 'form#search-form button[type="submit"]';
  this.waitForSelector(sel, function()
  {
    this.click(sel);//Clicking on the search button
  });
...
var goToSearchPage = function()
{
  ...
  this.waitForSelector(sel, function()
  {
    this.click(sel);//Clicking on the search link
